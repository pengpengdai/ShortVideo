...
func (s *AIService) CreateConfig(req *CreateAIConfigRequest) (*models.AIServiceConfig, error) {
	// DeepSeek 仅支持文本生成（剧本、角色提取等），不支持图片和视频
	if req.Provider == "deepseek" && req.ServiceType != "text" {
		return nil, errors.New("DeepSeek 仅支持文本服务，不支持图片和视频生成")
	}
...
func (s *AIService) UpdateConfig(configID uint, req *UpdateAIConfigRequest) (*models.AIServiceConfig, error) {
	...
	// DeepSeek 仅支持文本服务
	if req.Provider == "deepseek" && config.ServiceType != "text" {
		return nil, errors.New("DeepSeek 仅支持文本服务，不支持图片和视频生成")
	}
...
func (s *AIService) TestConnection(req *TestConnectionRequest) error {
	...
	case "gemini", "google":
		...
		case "openai", "chatfire", "deepseek":
		// OpenAI 格式（包括 chatfire、deepseek 等）
